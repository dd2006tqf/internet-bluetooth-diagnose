...
- [x] 1 补齐 ServerContext 线程句柄（jitter/rssi/bt）并使 rtt_thread 实际赋值
...
- [x] 2 ServerContext 资源 RAII（WeakNetMgr/DbusService/DBus 连接）并由析构释放
...
- [x] 6 接口列表唯一事实源收口（dbus 改读 WeakNetMgr，修 ctx->iface_list 无锁写）
  - Covers: `specs/weaknet-server/spec.md` | `ADDED` | `D-Bus 信号发射线程安全` | `信号发射加互斥`
  - Verify: `build`
