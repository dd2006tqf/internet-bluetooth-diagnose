...
- [x] 2 network_quality_thread 接入冲突检测
...
- [x] 4 BtMonitor refreshAudioTransports + 评分
...
- [x] 8 bt_audio_fusion 状态机/时间加权/stall
...
- [x] 9 BtMonitor 接入融合层 + BPF Map 开关
  - Covers: `specs/bluetooth-monitoring/spec.md` | `ADDED` | `D-Bus eBPF Fusion` | `时间加权 Delay`
  - Verify: `build`, `test`
  - Operation: 修改 `bt_monitor.cpp`，`onTransportStateChanged` 写 `active_sessions.enabled`，`refreshDeviceStates` 内调 `BtAudioFusion.evaluate`
  - Requirement: REQ-FUSION、REQ-EVENT-ROUTING
  - TDD: RED（融合层接入断言）→ GREEN → REGRESSION
