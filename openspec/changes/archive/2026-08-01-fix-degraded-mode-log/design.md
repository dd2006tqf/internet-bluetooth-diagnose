# Design
## Overview
在 startTrafficAnalysis 中调用 isDegradedMode()，若为 true 则 LOG_WARNING。

<!-- autoai:tdd-policy:v1 -->
```json
{"schema_version":1,"default":"required","exceptions":[{"id":"exc-1","category":"observability_only","task_ids":["1"],"paths":["server/src/weak_netmgr.cpp"],"reason":"日志输出是 observable 行为变更，无需 RED/GREEN","alternative_verify_kinds":["build"],"exit_condition":"构建通过后归档"}]}
```
<!-- /autoai:tdd-policy:v1 -->
