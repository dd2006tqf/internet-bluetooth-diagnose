...
## Verification Patterns for This Project
...
```bash
# x86 build
cmake -B build -DCMAKE_BUILD_TYPE=Debug -DBUILD_EBPF=OFF && cmake --build build -j$(nproc)
# Expected: exit 0
```

**ARM64 build:**
```bash
docker exec weaknet-arm64-dev bash -c 'cd /src && cmake -B build -DCMAKE_BUILD_TYPE=Debug && cmake --build build -j1'
# Expected: exit 0
```
