...
**我做什么**：执行 `scripts/change_archive.sh <name>`，自动完成：
1. 变更从 `openspec/changes/<name>/` 移到 `openspec/changes/archive/<日期>-<name>/`
2. 主 spec 更新，active-change selector 清空
3. 归档成功后把全部改动提交到 git（确保工作区干净）

> 归档**只能**走 `scripts/change_archive.sh`（见 CLAUDE.md 铁律）。
> 该包装器会重新校验任务、strict 校验、指纹、footprint、surface 报告与
> Evaluation Pass，任一不通过即拒绝归档。
...
| 归档 | `bash scripts/change_archive.sh <name>` |
| 规划校验 | `openspec validate --strict` |
| 集成完备性检查 | `bash scripts/integration_surface_check.sh --plan-check` |
| 审计汇总 | `bash scripts/audit_summary.sh <name>` |
| 中止变更（逃生舱） | `bash scripts/change_abort.sh <name> [--purge]` |

---

## 逃生舱：中止变更

如果变更卡住或需要放弃，用 `change_abort.sh` 优雅退出：

```bash
# 仅清理 harness 产物，保留规划文档和工作区代码
bash scripts/change_abort.sh <name>

# 彻底删除 change 目录（含规划文档），保留工作区代码
bash scripts/change_abort.sh <name> --purge
```

中止后：
- 工作区源码改动保留（你可以手动 `git commit` 或 `git checkout`）
- harness 产物（verification.json、证据文件等）已清理
- active-change selector 已清理
...
```bash
bash scripts/audit_summary.sh <name>
```

读取 `verification.json`，在 `harness/audit-summary.md` 生成汇总：每个任务的 TDD 闭环状态、RED/GREEN/REGRESSION 数量、是否闭环。
...
| 文档 | 定位 | 谁看 |
|---|---|---|
| **工作流使用指南.md**（本文件） | 入口：使用流程与协作方式 | 项目作者 |
| `openspec.md` | OpenSpec 在 harness 中的角色 | 了解规划数据平面 |
| `workflow.md` | 工作流绑定规则（内部契约） | 了解角色分离约束 |
| `implementation-economy.md` | 实现经济契约（预算/分类） | 写 design.md 时参考 |
| `evaluation.md` | 独立验收契约 | Evaluator 阶段参考 |
| `check-rules.md` | 仓库检查规则 | 了解校验项 |
| `quick-brief.md` | Quick Brief 策略 | 长文档精简策略 |
| `tooling.md` | 工具使用约束 | 了解脚本调用规范 |
| `cpp.md` | C++ 实现规则 | 写 C++ 代码时参考 |
| `testing.md` | 测试规则 | 写测试时参考 |
| `build.md` | 构建规则 | 改 CMake 构建元数据时参考 |
| `golden-principles.md` | 黄金原则 | 通用原则 |
| `rca.md` | 根因分析 | 调试失败时参考 |

**一般只需读本文件即可**；其他文档是 harness 内部契约，按需查阅。
...
5. **文档命名均用中文**（项目文档使用中文文件名，如 `测试结果.md`、`架构设计.md`；harness 框架文件如 `AGENTS.md`、`CLAUDE.md`、`docs/ai/` 下模板、`prompts/` 下提示词因被 `manifest_policy.js` 硬编码引用，保持原名不改）
6. **改 design.md 非字段不破坏 TDD 证据**（planning_fingerprint 只哈希 TDD Policy 块，改 Implementation Economy 或 Integration Completeness 不会让证据失效）
7. **卡住时用逃生舱**（`bash scripts/change_abort.sh <name>` 优雅中止，保留代码改动）
