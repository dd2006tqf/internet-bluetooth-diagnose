...
| P0 | 服务主循环 | `server.cpp` | 8 个监控线程的启动和协调 |
...
| P1 | 蓝牙监控 | `bt_monitor.cpp` | BlueZ D-Bus API + 事件路由 |
| P2 | 质量评估 | `network_quality_assessor.cpp` | 加权评分算法 |
| P2 | AI 诊断 | `AI-assisted analysis/` | RAG + LangChain + FAISS |

### 2.2 监控线程一览

服务端启动 8 个监控线程，详细职责和采集周期见 [架构设计文档 - 第四章](架构设计.md#四服务端职责)。

| 线程 | 采集周期 | 输出信号 |
|------|----------|----------|
| 接口监控 | 事件驱动 | `InterfaceChanged` |
| 当前网卡 | 事件驱动 | `ConnectionModeChanged` |
| RTT 监控 | 10s | `RttChanged` |
| RSSI 监控 | 10s | `RssiChanged` |
| TCP 丢包 | 10s | `TcpLossRateChanged` |
| 流量分析 | 10s | `Changed` |
| 质量评估 | 15s | `NetworkQualityChanged` |
