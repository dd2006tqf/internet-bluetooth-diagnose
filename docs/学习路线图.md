...
> **更新日期**：2026-09-03。当前代码库已有 6 个 eBPF 程序、15 个监控器插件 + 1 个历史持久化线程、`MonitorManager` 统一生命周期。
...
| 优先级 | 模块 | 关键文件 | 学习要点 |
|--------|------|----------|----------|
| P0 | 生命周期中枢 | `monitor_manager.hpp` | `MonitorManager` 协调 15 个插件 + 6 个 eBPF 监控器 |
| P0 | 服务主循环 | `server.cpp` | 15 个监控器插件的启动和协调 |
...
### 2.2 监控线程一览（当前 15 个监控器插件）

服务端启动 15 个监控器插件（各自拥有独立 worker），详细职责和采集周期见 [架构设计文档 - 第四章](架构设计.md#五服务端职责)。
...
```
                    ...
                    └────┬─────┘
                         │
                  ┌──────▼──────┐
                  │ server.cpp  │ 启动 15 个监控器插件 + D-Bus 主循环
                  └──────┬──────┘
                         │
                  ┌──────▼──────┐
