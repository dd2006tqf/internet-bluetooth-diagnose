...
case "${1:-}" in
  backend)
    ...
    .venv/bin/python -B -m mypy --cache-dir .mypy_cache \
      ...
      src/industrial_ops_agent/deployment/kserve.py \
      src/industrial_ops_agent/api/routes/model_deployments.py
    echo KSERVE_GPU_AUTOSCALING_BACKEND_STATIC_OK
    ;;
  frontend)
    (
      cd web
    ...
    echo KSERVE_GPU_AUTOSCALING_FRONTEND_STATIC_OK
    ;;
  gitops)
    helm version --short
    helm lint infra/helm/industrial-ops-observability
    ...
    echo KSERVE_GPU_AUTOSCALING_GITOPS_OK
    ;;
  *)
