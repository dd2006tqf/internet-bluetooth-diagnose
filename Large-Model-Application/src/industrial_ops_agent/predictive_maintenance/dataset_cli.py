"""Airflow-facing entrypoint for governed telemetry sequence snapshots."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-telemetry-dataset")
    subcommands = parser.add_subparsers(dest="command", required=True)
    build = subcommands.add_parser("build", help="build one immutable telemetry snapshot")
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = TelemetryDatasetPipelineService(
        database,
    ...
    try:
        result = service.build_snapshot(
            TenantContext(args.tenant_id, "airflow-telemetry-data-pipeline"),
