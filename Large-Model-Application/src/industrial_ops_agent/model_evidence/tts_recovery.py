...
from industrial_ops_agent.model_evidence.tts_contracts import TTS_HARD_GATES
from industrial_ops_agent.model_evidence.tts_legacy_files import (
    LegacyTtsEvidenceFiles,
    legacy_ascii_digest,
)

_LEGACY_SOURCE_PATH = "src/industrial_ops_agent/simulation/tts_recovery_value_lab.py"
...
XVECTOR_RELATIVE = PREDECESSOR_OUTPUT_RELATIVE / "inputs" / "spkrec-xvect-voxceleb-56895a2d"


TRAINING_DATASET_RELATIVE = PREDECESSOR_OUTPUT_RELATIVE / "inputs" / "industrial-safety-voice-v1"


GOLD_DATASET_RELATIVE = OUTPUT_RELATIVE / "inputs" / "industrial-safety-tts-final-gold-v3"


PREDECESSOR_RETIREMENT_RELATIVE = PREDECESSOR_OUTPUT_RELATIVE / "gold-v2-retirement.json"
...
GOLD_MANIFEST_SHA256 = "59f17ced8e38e9efec937d8f921005ab05e1c041d20ff72abd9de97176cb1bf5"


PREDECESSOR_RETIREMENT_SHA256 = "e723b35efc2154d7d2fd770f048cb324593b9c74ae65f673163b6e8f2cf1dad7"
...
_FILES = LegacyTtsEvidenceFiles(TtsRecoveryValueLabError)
...
def planned_run_id(repo_root: Path, image_digest: str) -> str:
    ...
    identity = {
        ...
        "predecessor_retirement_sha256": file_sha256(paths["predecessor_retirement"]),
        "speaker_embedding_sha256": file_sha256(paths["speaker_embedding"]),
    }
    return f"tts-recovery-{legacy_ascii_digest(identity)[:20]}"
...
def verify_tts_recovery_value(
    ...
    report = TtsRecoveryValueReport.model_validate_json(report_path.read_text(encoding="utf-8"))
...
def verify_tts_recovery_rejection(
    ...
    report_path = (
        ...
        else _FILES.inside_file(root, rejection_path)
    )
    report = TtsRecoveryRejectionReport.model_validate_json(report_path.read_text(encoding="utf-8"))
    unsigned = report.model_dump(mode="json", exclude={"evidence_chain_sha256"})
    if report.evidence_chain_sha256 != legacy_ascii_digest(unsigned):
    ...
    _verify_file_evidence(root, report.dataset.retired_predecessor)
    _verify_file_evidence(root, report.dataset.speaker_embedding)
    formal_evidence = FileEvidence.model_validate(report.evaluation.get("formal_report"))
    observations_evidence = FileEvidence.model_validate(report.evaluation.get("observations"))
    dataset_evidence = FileEvidence.model_validate(report.evaluation.get("dataset_receipt"))
    ...
    formal_document = _FILES.load_object(formal_path)
...
def verify_tts_recovery_outcome(
    ...
    root = repo_root.resolve(strict=True)
    path = _FILES.inside_file(root, outcome_path)
    document = _FILES.load_object(path)
...
def _input_paths(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "predecessor_retirement": _FILES.inside_file(root, PREDECESSOR_RETIREMENT_RELATIVE),
    }
...
def _verify_inputs(paths: dict[str, Any]) -> None:
    ...
    if (
        ...
        or file_sha256(paths["predecessor_retirement"]) != PREDECESSOR_RETIREMENT_SHA256
...
def _latest_outcome_path(root: Path) -> Path:
    latest_path = _FILES.inside_file(root, OUTPUT_RELATIVE / "latest-outcome.json")
    latest = _FILES.load_object(latest_path)
    value = latest.get("report")
    if not isinstance(value, str) or not value:
...
def _retired_source(root: Path) -> dict[str, Any]:
    return retired_source_binding(root, _LEGACY_SOURCE_PATH, error_type=TtsRecoveryValueLabError)
