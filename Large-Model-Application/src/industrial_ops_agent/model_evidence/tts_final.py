...
_LEGACY_SOURCE_PATH = "src/industrial_ops_agent/simulation/tts_final_value_lab.py"

SCHEMA_VERSION: Literal["enterprise-tts-final-value-lab/v6"] = "enterprise-tts-final-value-lab/v6"
...
GOLD_RELATIVE = OUTPUT_RELATIVE / "inputs/industrial-safety-tts-final-gold-v7/manifest.json"
...
def planned_run_id(repo_root: Path, image_digest: str) -> str:
    ...
    return f"tts-final-{tts_files.digest(identity)[:20]}"
...
def verify_calibrated_tts_value(
    ...
    retirement = _files.verify_gold_retirement(
        root, root / report.gold_retirement_path, gold_relative=GOLD_RELATIVE, version=7
    )
