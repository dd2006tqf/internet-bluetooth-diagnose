"""Actual-GPU contract-segmented TTS enterprise-value lab v4.
...
The retired v6 suite is used only as a disclosed development regression set;
validation must pass before the fresh, disjoint, single-use v7 Gold is opened.
"""
...
SCHEMA_VERSION: Literal["enterprise-tts-final-value-lab/v6"] = (
    "enterprise-tts-final-value-lab/v6"
)
...
OUTPUT_RELATIVE = Path("artifacts/m7-tts-final-value-lab")
VALIDATION_RELATIVE = OUTPUT_RELATIVE / "inputs/industrial-safety-tts-validation-v5.json"
...
GOLD_RELATIVE = (
    OUTPUT_RELATIVE / "inputs/industrial-safety-tts-final-gold-v7/manifest.json"
)
GOLD_RETIREMENT_RELATIVE = OUTPUT_RELATIVE / "gold-v7-retirement.json"
...
CONTAINER_NAME = "ioap-tts-final-value-lab"
...
_CONFIG: dict[str, Any] = {
    "method": "SPEECHT5_CONTRACT_SEGMENTED_SAFE_VOICE_SELECTION",
...
class DatasetEvidence(_ClosedModel):
    ...
    retired_v2_v3_v4_v5_v6_reused_as_formal_gold: Literal[False] = False
    retired_v6_used_as_disclosed_regression: Literal[True] = True
...
class RuntimeEvidence(_ClosedModel):
    ...
    method: Literal["SPEECHT5_CONTRACT_SEGMENTED_SAFE_VOICE_SELECTION"] = (
        "SPEECHT5_CONTRACT_SEGMENTED_SAFE_VOICE_SELECTION"
    )
...
class TtsCalibratedValueReport(_ClosedModel):
    schema_version: Literal["enterprise-tts-final-value-lab/v6"] = SCHEMA_VERSION
    ...
    status: Literal[
        "TTS_FINAL_ACTUAL_GPU_ENTERPRISE_VALUE_PASSED",
        "TTS_FINAL_ACTUAL_GPU_CANDIDATE_REJECTED",
    ]
    decision: Literal[
        "TTS_CANDIDATE_ELIGIBLE_FOR_MODEL_RELEASE_DRAFT",
        "DO_NOT_REGISTER_DEPLOY_OR_REUSE_GOLD",
    ]
    run_id: str = Field(pattern=r"^tts-final-[0-9a-f]{20}$")
    source: FileBinding
...
_VALIDATION_ROWS: tuple[dict[str, Any], ...] = _RETIRED_GOLD_V6_ROWS + (
    {
...
def validation_document() -> dict[str, Any]:
    return {
        "schema_version": "industrial-safety-tts-validation/v5",
        "suite_id": "industrial-safety-tts-validation-v5",
        "classification": CLASSIFICATION,
        "purpose": "DEVELOPMENT_PREFLIGHT_ONLY",
        "formal_gold": False,
        "project_enterprise_use_authorized": True,
        "retired_gold_v6_role": "DISCLOSED_DEVELOPMENT_REGRESSION_ONLY",
        "retired_gold_v6_reused_as_formal_gold": False,
...
def formal_gold_document() -> dict[str, Any]:
    return {
        "schema_version": "industrial-safety-tts-final-gold/v7",
        "dataset_id": "industrial-safety-tts-final-gold-v7",
        "classification": CLASSIFICATION,
        ...
        "supersedes_dataset_id": "industrial-safety-tts-final-gold-v6",
        "selection_policy": {
...
def preflight_calibrated_tts(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "enterprise-tts-final-preflight/v6",
        "classification": CLASSIFICATION,
        "status": "TTS_FINAL_PREFLIGHT_PASSED",
...
def assert_gold_available(repo_root: Path) -> None:
    ...
    if retirement.exists():
        ...
        raise TtsCalibratedValueLabError("TTS Gold v7 is already retired")
...
def planned_run_id(repo_root: Path, image_digest: str) -> str:
    ...
    return f"tts-final-{_digest(identity)[:20]}"
...
def execute_worker(repo_root: Path, *, image_digest: str) -> Path:
    ...
    try:
        ...
        validation_document_value = {
            "schema_version": "enterprise-tts-final-validation/v6",
            "classification": CLASSIFICATION,
            "run_id": run_id,
        ...
        try:
            ...
            failure_document = {
                "schema_version": "enterprise-tts-final-gold-stage-failure/v1",
                "classification": CLASSIFICATION,
                "status": "TTS_FINAL_GOLD_NO_SAFE_VARIANT_REJECTED",
                "run_id": run_id,
        ...
        report_value: dict[str, Any] = {
            ...
            "status": (
                "TTS_FINAL_ACTUAL_GPU_ENTERPRISE_VALUE_PASSED"
                if accepted
                else "TTS_FINAL_ACTUAL_GPU_CANDIDATE_REJECTED"
            ),
            "decision": (
                "TTS_CANDIDATE_ELIGIBLE_FOR_MODEL_RELEASE_DRAFT"
                if accepted
            ...
            "dataset": {
                ...
                "project_enterprise_use_authorized": True,
                "enterprise_production_data": False,
                "retired_v2_v3_v4_v5_v6_reused_as_formal_gold": False,
                "retired_v6_used_as_disclosed_regression": True,
...
def execute_validation_probe(repo_root: Path, *, image_digest: str) -> Path:
    ...
    try:
        ...
        document = {
            "schema_version": "enterprise-tts-final-validation-probe/v6",
            "classification": CLASSIFICATION,
            "status": (
                "TTS_FINAL_GPU_VALIDATION_PASSED"
                if not result.failures
                else "TTS_FINAL_GPU_VALIDATION_REJECTED"
            ),
            "run_id": run_id,
        ...
        document = {
            "schema_version": "enterprise-tts-final-validation-probe/v6",
            "classification": CLASSIFICATION,
            "status": "TTS_FINAL_GPU_VALIDATION_REJECTED",
            "run_id": run_id,
            "actual_gpu_inference": True,
            "formal_gold_consumed": False,
            "failure": exc.document,
...
def _materialize_candidate(
    ...
    policy = {
        ...
        "formal_gold_v7_derived": False,
        "retired_gold_v6_regression_used": True,
        "runtime_source": "industrial_ops_agent.simulation.tts_final_value_lab",
    }
...
def _verify_dataset_independence(
    ...
    for document in prior_documents:
        ...
        if gold_ids & prior_ids or gold_texts & prior_texts:
            raise TtsCalibratedValueLabError("TTS Gold v7 reuses predecessor data")
    if (
        len(validation_ids) != 12
        or len(gold_ids) != 10
        or gold.get("dataset_id") != "industrial-safety-tts-final-gold-v7"
...
def _write_gold_retirement(
    ...
    document = {
        "schema_version": "industrial-safety-tts-gold-retirement/v7",
        "status": (
        ...
        "run_id": report.run_id,
        "gold_dataset_id": "industrial-safety-tts-final-gold-v7",
...
def _verify_gold_retirement(root: Path, path: Path) -> dict[str, Any]:
    ...
    if (
        retirement.get("schema_version") != "industrial-safety-tts-gold-retirement/v7"
        or retirement.get("gold_dataset_id") != "industrial-safety-tts-final-gold-v7"
        or retirement.get("gold_evaluation_pass_count") != 1
        or retirement.get("reuse_permitted") is not False
    ):
        raise TtsCalibratedValueLabError("TTS Gold v7 retirement changed")
    gold_path = _inside_file(root, GOLD_RELATIVE)
    if retirement.get("gold_manifest_sha256") != _file_sha256(gold_path):
        raise TtsCalibratedValueLabError("TTS Gold v7 retirement manifest changed")
    ...
    if isinstance(outcome_value, str):
        ...
        if retirement.get("outcome_sha256") != _file_sha256(outcome):
            raise TtsCalibratedValueLabError("TTS Gold v7 retirement outcome changed")
    return retirement
...
def _write_latest(root: Path, path: Path, report: TtsCalibratedValueReport) -> None:
    _write_json(
        ...
        {
            "schema_version": "enterprise-tts-final-latest/v1",
            "status": report.status,
            "run_id": report.run_id,
...
def _write_latest_outcome(
    ...
    _write_json(
        ...
        {
            "schema_version": "enterprise-tts-final-latest-outcome/v1",
            "status": report.status,
            "run_id": report.run_id,
