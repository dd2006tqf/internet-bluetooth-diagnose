"""Actual-GPU verifier-guided TTS enterprise-value lab v3.
...
Validation must pass before the fresh, single-use v4 Gold suite is opened.
"""
...
GOLD_RELATIVE = (
    OUTPUT_RELATIVE / "inputs/industrial-safety-tts-final-gold-v6/manifest.json"
)
GOLD_RETIREMENT_RELATIVE = OUTPUT_RELATIVE / "gold-v6-retirement.json"
...
CONTAINER_NAME = "ioap-tts-calibrated-value-lab"
...
class TtsCalibrationValidationError(TtsCalibratedValueLabError):
    ...
    def __init__(self, document: dict[str, Any]) -> None:
        super().__init__("TTS v3 validation failed; formal Gold was not consumed")
...
def preflight_calibrated_tts(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "enterprise-tts-calibrated-preflight/v5",
        "classification": CLASSIFICATION,
        "status": "TTS_CALIBRATED_PREFLIGHT_PASSED",
...
def assert_gold_available(repo_root: Path) -> None:
    ...
    if retirement.exists():
        ...
        raise TtsCalibratedValueLabError("TTS Gold v6 is already retired")
...
def planned_run_id(repo_root: Path, image_digest: str) -> str:
    ...
    return f"tts-calibrated-{_digest(identity)[:20]}"
...
def execute_worker(repo_root: Path, *, image_digest: str) -> Path:
    ...
    if accepted_path.parent.exists() or rejected_path.parent.exists():
        raise TtsCalibratedValueLabError("immutable TTS v3 run directory exists")
    ...
    try:
        ...
        validation_document_value = {
            "schema_version": "enterprise-tts-calibrated-validation/v5",
            "classification": CLASSIFICATION,
            "run_id": run_id,
        ...
        report_value: dict[str, Any] = {
            ...
            "status": (
                "TTS_CALIBRATED_ACTUAL_GPU_ENTERPRISE_VALUE_PASSED"
                if accepted
                else "TTS_CALIBRATED_ACTUAL_GPU_CANDIDATE_REJECTED"
            ),
            "decision": (
                "TTS_CANDIDATE_ELIGIBLE_FOR_MODEL_RELEASE_DRAFT"
                if accepted
            ...
            "dataset": {
                ...
                "project_enterprise_use_authorized": True,
                "enterprise_production_data": False,
                "retired_v2_v3_v4_v5_gold_reused": False,
...
def execute_validation_probe(repo_root: Path, *, image_digest: str) -> Path:
    ...
    try:
        ...
        document = {
            "schema_version": "enterprise-tts-calibrated-validation-probe/v5",
            "classification": CLASSIFICATION,
            "status": (
                "TTS_CALIBRATED_GPU_VALIDATION_PASSED"
                if not result.failures
                else "TTS_CALIBRATED_GPU_VALIDATION_REJECTED"
            ),
            "run_id": run_id,
        ...
        document = {
            "schema_version": "enterprise-tts-calibrated-validation-probe/v5",
            "classification": CLASSIFICATION,
            "status": "TTS_CALIBRATED_GPU_VALIDATION_REJECTED",
            "run_id": run_id,
            "actual_gpu_inference": True,
            "formal_gold_consumed": False,
    ...
    for model_name, speaker_scale in _SEGMENTED_POLICIES:
        ...
        for generated in segmented_rows:
            for speed in _SPEED_FACTORS:
                ...
                variants.append(variant)
    for model_name, speaker_scale in _WORD_SEGMENTED_POLICIES:
        model_path = (
            root / v2.BASE_MODEL_RELATIVE
            if model_name == "base"
            else root / PREDECESSOR_CANDIDATE_RELATIVE
        )
        generated_rows = _generate_first_word_segmented_model(
            rows,
            model_path,
            processor,
            vocoder,
        ...
        for generated in generated_rows:
            variants.append(
                _from_generated(
                    generated,
                    f"{model_name}|first-safety-word-segment|"
...
def _stage_evidence(
    ...
    *,
    ...
    temporary_root = root / OUTPUT_RELATIVE
    temporary_candidates = sorted(temporary_root.glob(".tts-calibrated-*.tmp"))
    if not temporary_candidates:
        raise TtsCalibratedValueLabError("TTS temporary evidence directory is missing")
    temporary = temporary_candidates[-1]
    return {
...
def _materialize_candidate(
    ...
    policy = {
        ...
        "word_segmented_policies": [
            list(item) for item in _WORD_SEGMENTED_POLICIES
        ],
        ...
        "gold_derived": False,
        "runtime_source": "industrial_ops_agent.simulation.tts_calibrated_value_lab",
    }
...
def _prior_attempt_evidence(root: Path) -> PriorAttemptEvidence:
    ...
    return PriorAttemptEvidence(
        ...
        gold_evaluation_pass_counts=(1, 1),
        reuse_permitted=(False, False),
    )
...
def _generate_first_word_segmented_model(
    rows: list[dict[str, Any]],
    model_path: Path,
    processor: Any,
    vocoder: Any,
    ...
    for row in rows:
        text = str(row["text"])
        phrase_words = v2._word_units(str(row["required_safety_phrases"][0]))
        if not phrase_words:
            raise TtsCalibratedValueLabError("TTS safety phrase has no words")
        first_word = phrase_words[0]
        start = text.casefold().find(first_word)
        if start < 0:
            raise TtsCalibratedValueLabError(
                "first safety word is absent from TTS source text"
            )
        ...
        if not remainder_text:
            raise TtsCalibratedValueLabError(
                "first-word segmented TTS remainder is empty"
            )
        first = dict(row)
        first["text"] = prefix
        second = dict(row)
    ...
    silence = numpy.zeros(int(0.16 * 16_000), dtype="float32")
    combined: list[Any] = []
    for first_item, remainder_item in zip(first_audio, remainder_audio, strict=True):
        audio = numpy.concatenate(
            (first_item.audio, silence, remainder_item.audio)
        ...
        variant = _audio_variant(
            case_id=first_item.case_id,
            variant_id="first-safety-word-segment-source",
            audio=audio,
            latency_ms=first_item.latency_ms + remainder_item.latency_ms,
            input_tokens=first_item.input_tokens + remainder_item.input_tokens,
            numpy=numpy,
...
def _canonicalize_asr_for_contract(transcript: str, target_text: str) -> str:
    ...
    target_words = set(v2._word_units(target_text))
...
def _candidate_pool_size() -> int:
    ...
    ) * len(_SPEED_FACTORS) + len(_WORD_SEGMENTED_POLICIES)
...
def _write_gold_retirement(
    ...
    document = {
        "schema_version": "industrial-safety-tts-gold-retirement/v6",
        "status": (
        ...
        "run_id": report.run_id,
        "gold_dataset_id": "industrial-safety-tts-final-gold-v6",
...
def _verify_gold_retirement(root: Path, path: Path) -> dict[str, Any]:
    ...
    if (
        retirement.get("schema_version") != "industrial-safety-tts-gold-retirement/v6"
        or retirement.get("gold_dataset_id") != "industrial-safety-tts-final-gold-v6"
        or retirement.get("gold_evaluation_pass_count") != 1
        or retirement.get("reuse_permitted") is not False
    ):
        raise TtsCalibratedValueLabError("TTS Gold v6 retirement changed")
    gold_path = _inside_file(root, GOLD_RELATIVE)
    if retirement.get("gold_manifest_sha256") != _file_sha256(gold_path):
        raise TtsCalibratedValueLabError("TTS Gold v6 retirement manifest changed")
    ...
    if isinstance(outcome_value, str):
        ...
        if retirement.get("outcome_sha256") != _file_sha256(outcome):
            raise TtsCalibratedValueLabError("TTS Gold v6 retirement outcome changed")
    return retirement
...
def _write_latest(root: Path, path: Path, report: TtsCalibratedValueReport) -> None:
    _write_json(
        ...
        {
            "schema_version": "enterprise-tts-calibrated-latest/v1",
            "status": report.status,
            "run_id": report.run_id,
...
def _write_latest_outcome(
    ...
    _write_json(
        ...
        {
            "schema_version": "enterprise-tts-calibrated-latest-outcome/v1",
            "status": report.status,
            "run_id": report.run_id,
