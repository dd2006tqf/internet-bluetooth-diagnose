"""CLI for the one-shot actual-GPU DPO Agent Runtime value evaluation."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-dpo-agent-runtime-value-lab")
...
def run(argv: list[str] | None = None) -> None:
    ...
    if args.command == "prepare":
        ...
        print("DPO_AGENT_RUNTIME_GOLD_FROZEN")
        print(path)
        return
    if args.command == "worker":
        path = execute_worker(root, image_digest=args.image_digest)
        report = verify_dpo_agent_runtime_value(root, path)
    elif args.command == "verify":
        report = verify_dpo_agent_runtime_value(root)
...
def _run_container(root: Path) -> tuple[DpoAgentRuntimeValueReport, Path]:
    ...
    for path in (
        root / "artifacts/m7-dpo-agent-runtime-value-lab/runs" / run_id / "acceptance.json",
        root
        ...
        if path.is_file():
            return verify_dpo_agent_runtime_value(root, path), path
    if _container_exists(root):
        raise DpoAgentRuntimeValueLabError(
            f"DPO Agent Runtime lab container already exists: {CONTAINER_NAME}"
        )
    command = [
        "docker",
        "run",
        ...
        "-B",
        "-m",
        "industrial_ops_agent.simulation.dpo_agent_runtime_value_lab_cli",
        "worker",
        "--repo-root",
        "/workspace",
        "--image-digest",
    ...
    try:
        ...
        raise DpoAgentRuntimeValueLabError(
            "actual GPU DPO Agent Runtime container failed"
        ) from exc
    finally:
        if _container_exists(root):
        ...
        if _container_exists(root):
            raise DpoAgentRuntimeValueLabError(
                "DPO Agent Runtime lab container cleanup failed"
            )
    report = verify_dpo_agent_runtime_value(root)
...
def _print_report(report: DpoAgentRuntimeValueReport, path: Path) -> None:
    print(report.status)
    print(report.classification)
    print(f"CANDIDATE_ACCEPTED={str(report.candidate_accepted).lower()}")
    print(
        "BASELINE_SEMANTIC_SCORE="
        f"{report.evaluation.baseline.average_semantic_score:.6f}"
    )
    print(
        "CANDIDATE_SEMANTIC_SCORE="
        f"{report.evaluation.candidate.average_semantic_score:.6f}"
    )
...
def _capture_checked(command: list[str], *, cwd: Path) -> str:
    try:
        ...
        raise DpoAgentRuntimeValueLabError(f"command failed: {command[0]}") from exc
