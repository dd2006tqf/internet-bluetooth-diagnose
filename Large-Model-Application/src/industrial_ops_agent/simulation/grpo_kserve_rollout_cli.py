"""Execute the actual-GPU GRPO ModelRelease on the local KServe control plane."""
...
WORKER_CONTAINER = "ioap-grpo-kserve-gpu-worker"
...
def execute_rollout(repo_root: Path) -> Path:
    ...
    if acceptance_path.is_file():
        verify_grpo_kserve_rollout(root, acceptance_path)
        return acceptance_path
    output = root / OUTPUT_RELATIVE
    if output.exists():
        raise GrpoKServeRolloutError(
            "GRPO rollout output exists without a verified acceptance receipt"
        )
    ...
    runtime_value = verify_grpo_agent_runtime_value(root, runtime_path)
    training_path = root / Path(runtime_value.grpo_source.acceptance.path)
    training = verify_grpo_post_training(root, training_path)
    if (
        not runtime_value.candidate_accepted
        or runtime_value.grpo_source.run_id != training.run_id
        or runtime_value.grpo_source.adapter_bundle_sha256 != training.adapter.bundle_sha256
    ):
        raise GrpoKServeRolloutError("GRPO release source is not accepted")
    ...
    try:
        ...
        stage_reports: list[GrpoRolloutStageEvidence] = []
        ...
        with ExitStack() as stack:
            ...
            for stage in ("SHADOW", "CANARY_5", "CANARY_25", "ROLLED_BACK"):
                ...
                state.deployment = state.deployment_service.reconcile(
                    ...
                    request_id=f"reconcile-grpo-{stage.lower()}",
                )
                if (
                    state.deployment.deployment.status != "READY"
                    or state.deployment.deployment.current_stage != stage
                ):
                    raise GrpoKServeRolloutError(f"ModelRelease deployment did not enter {stage}")
                ...
                if stage != "ROLLED_BACK":
                    ...
                    if stage in {"SHADOW", "CANARY_5"}:
                        state.deployment = state.deployment_service.promote(
                            ...
                            request_id=f"promote-grpo-{stage.lower()}",
                        )
                    else:
                        state.deployment = state.deployment_service.request_rollback(
                            ...
                            request_id="rollback-grpo-after-canary-25",
                        )
        ...
        unsigned: dict[str, Any] = {
            "schema_version": "enterprise-grpo-kserve-rollout/v1",
            ...
            "cleanup": {
                ...
                "grpo_kubernetes_resources_removed": resources_removed,
        ...
        if tail:
            print("GRPO_GPU_WORKER_LOG_TAIL_BEGIN", flush=True)
            print(tail, flush=True)
            print("GRPO_GPU_WORKER_LOG_TAIL_END", flush=True)
        print(f"GRPO_KSERVE_ROLLOUT_ERROR={exc}", flush=True)
        raise
    finally:
        if resources_applied and not resources_removed and cluster_started:
            _delete_grpo_resources(root, check=False)
...
def _prepare_release_state(root: Path, state_path: Path) -> _ReleaseState:
    ...
    auditor = SecurityAuditor(
        InMemorySecurityAuditSink(),
        hash_key=b"grpo-local-kserve-release",
    )
    ...
    imported = EnterpriseModelAssetImportService(
        ...
        engineer,
        "LLM",
        idempotency_key="grpo-kserve-import-llm",
        request_id="grpo-kserve-import-llm",
    )
    ...
    releases.decide_approval(
        ...
        request_id="grpo-kserve-baseline-approve",
    )
    onboarding = EnterpriseReleaseOnboardingService(
        ...
        active_mcp_server_versions={},
        idempotency_key="grpo-kserve-candidate-release",
        request_id="grpo-kserve-candidate-release",
    )
    candidate = releases.validate(
        engineer,
        onboarding.release_id,
        expected_version=onboarding.release_version,
        request_id="grpo-kserve-candidate-validate",
    )
    pending = releases.submit_for_approval(
        engineer,
        onboarding.release_id,
        expected_version=candidate.release.version,
        request_id="grpo-kserve-candidate-submit",
    )
    if pending.approval is None:
        raise GrpoKServeRolloutError("candidate approval was not created")
    approved = releases.decide_approval(
        ...
        decision="APPROVED",
        reason="independent GRPO Agent Runtime and supply-chain review passed",
        request_id="grpo-kserve-candidate-approve",
    )
    if approved.approval is None or approved.approval.status != "APPROVED":
        raise GrpoKServeRolloutError("candidate approval did not pass")
    ...
    deployment = deployment_service.request_shadow(
        ...
        DeploymentPlan(
            ...
            serving_runtime_name="grpo-peft-runtime",
            artifact_uri_prefix="s3://project-enterprise-staging/grpo",
        ),
        expected_release_version=approved.release.version,
        request_id="grpo-kserve-request-shadow",
    )
...
def _record_stage_observation(
    ...
    *,
    ...
    stage_report: GrpoRolloutStageEvidence,
    ...
    return state.deployment_service.record_observation(
        ...
        ObservationInput(
            ...
            metrics=metrics,
            source_refs={
                "collector_version": "grpo-kserve-rollout/v1",
                "prometheus_endpoint_id": "grpo-agent-worker-metrics",
        ...
        expected_deployment_version=state.deployment.deployment.version,
        request_id=f"observe-grpo-{stage.lower()}",
    )
...
def _release_snapshot(state: _ReleaseState) -> dict[str, Any]:
    release = state.release_service.get(
        state.engineer,
        state.deployment.release.release_id,
        request_id="snapshot-grpo-release",
    )
    deployment = state.deployment_service.get(
        state.controller,
        state.deployment.release.release_id,
        request_id="snapshot-grpo-deployment",
    )
    if (
        ...
        raise GrpoKServeRolloutError("final ModelRelease state is invalid")
    ...
    if observation_stages != ("SHADOW", "CANARY_5", "CANARY_25") or any(
        ...
        raise GrpoKServeRolloutError("ModelRelease observation chain is invalid")
    return {
...
def _identity(role: Role) -> IdentityContext:
    ...
    return IdentityContext(
        subject_id=f"grpo-kserve-{role.value}",
        oidc_subject=f"oidc-grpo-kserve-{role.value}",
        tenant_id=TENANT_ID,
...
def _release_request(probe: dict[str, Any]) -> dict[str, Any]:
    return {
        "request_id": "grpo-release-direct-probe",
        "case_id": probe["case_id"],
        "equipment_id": probe["equipment_id"],
        "risk": probe["risk"],
        "prompt": probe["prompt"],
...
def _validate_candidate_quality(
    ...
    score = score_generated_completion(output, target)
    unsafe_hits = score.get("unsafe_pattern_hits")
    valid = bool(
        response.get("variant") == "candidate"
        and response.get("adapter_enabled") is True
        and response.get("structured_output_valid") is True
        and response.get("direct_object") is True
        and response.get("policy_consistent") is True
        and response.get("human_approval_required") is True
        and response.get("normalized_json") == probe["target"]
        and score["target_exact"] is True
        and score["direct_object"] is True
        and isinstance(unsafe_hits, list)
        and not unsafe_hits
    )
    if not valid:
        raise GrpoKServeRolloutError("candidate GRPO release probe failed")
    return {
        "candidate_structured_output_valid": True,
        "candidate_direct_object": True,
        "candidate_target_exact": True,
        "candidate_policy_consistent": True,
    ...
    with httpx.Client(timeout=timeout, trust_env=False) as client:
        for index in range(request_count):
            ...
            response = client.post(
                ...
                json={
                    **payload,
                    "request_id": f"grpo-{stage.lower()}-{index:05d}",
                },
            )
            latency_ms = max((time.perf_counter() - started) * 1_000, 0.001)
            response.raise_for_status()
            variant = response.headers.get("X-IOAP-GRPO-Variant", "")
            if variant not in {"stable", "candidate"}:
                raise GrpoKServeRolloutError("gateway GRPO variant is missing")
            document = response.json()
            if not isinstance(document, dict) or document.get("variant") != variant:
                raise GrpoKServeRolloutError("gateway GRPO response is invalid")
            output_sha = document.get("output_sha256")
            if not isinstance(output_sha, str) or len(output_sha) != 64:
                raise GrpoKServeRolloutError("gateway GRPO output digest is invalid")
            responses[variant] += 1
...
def _stage_evidence(
    ...
    *,
    ...
    if not passed or not output_sha256s or not latencies:
        raise GrpoKServeRolloutError(f"GRPO {stage} traffic evidence failed")
    return GrpoRolloutStageEvidence(
        stage=stage,
        passed=True,
...
def _start_worker(
    ...
    *,
    ...
    if _container_exists(root, WORKER_CONTAINER):
        raise GrpoKServeRolloutError(
            f"owned GRPO worker container already exists: {WORKER_CONTAINER}"
        )
    command = [
        "docker",
        "run",
        ...
        "-B",
        "-m",
        "industrial_ops_agent.agent.grpo_runtime",
        "--repo-root",
        "/workspace",
        "--model-path",
        "/models/base",
...
def _node_image_digest(root: Path) -> str:
    ...
    try:
        ...
        raise GrpoKServeRolloutError("Kind node returned invalid proxy image metadata") from exc
    status = document.get("status")
    if not isinstance(status, dict):
        raise GrpoKServeRolloutError("Kind node proxy image status is missing")
    ...
    if (
        ...
        raise GrpoKServeRolloutError("Kind node fixed proxy image binding changed")
    return digest
...
def _image_digest(root: Path, image: str) -> str:
    ...
    if not _valid_image_digest(value):
        raise GrpoKServeRolloutError("worker image digest is invalid")
    return value
...
def _wait_service_endpoint(root: Path, service: str, *, timeout_seconds: int) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if isinstance(subsets, list) and any(
            ...
            return
        time.sleep(1)
    raise GrpoKServeRolloutError(f"KServe endpoint is not ready: {service}")
...
def _wait_route(root: Path, *, timeout_seconds: int) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if route_condition_is_true(route, "Accepted") and route_condition_is_true(
            ...
            return
        time.sleep(0.5)
    raise GrpoKServeRolloutError("GRPO HTTPRoute was not accepted and resolved")
...
def _gateway_service(root: Path) -> tuple[str, str]:
    ...
    if len(matches) != 1 or not all(matches[0]):
        raise GrpoKServeRolloutError("Envoy Gateway service is not unique")
    return matches[0]
    ...
    *,
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise GrpoKServeRolloutError("GRPO GPU worker exited during startup")
        try:
            document = _get_json(f"{worker_url}/health/ready", timeout_seconds=2)
            if document.get("ready") is True:
                return
        except (httpx.HTTPError, GrpoKServeRolloutError):
            pass
        time.sleep(1)
    raise GrpoKServeRolloutError("GRPO GPU worker did not become ready")
    ...
    *,
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            ...
            raise GrpoKServeRolloutError(f"port-forward failed: {output[-512:]}")
        try:
            with socket.create_connection(("127.0.0.1", port), timeout=0.5):
                return
        except OSError:
            time.sleep(0.25)
    raise GrpoKServeRolloutError("port-forward did not become ready")
...
def _post_json(
    ...
    *,
    ...
    if not isinstance(document, dict):
        raise GrpoKServeRolloutError("GRPO response is invalid")
    return document
...
def _get_json(url: str, *, timeout_seconds: int) -> dict[str, Any]:
    with httpx.Client(timeout=timeout_seconds, trust_env=False) as client:
        ...
        document = response.json()
    if not isinstance(document, dict):
        raise GrpoKServeRolloutError("GRPO runtime response is invalid")
    return document
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if _variant_count(current, "request_counts", "candidate") - original >= minimum:
            return
        time.sleep(0.25)
    raise GrpoKServeRolloutError("Shadow traffic was not mirrored to GRPO candidate")
...
def _kubectl_json(root: Path, resource: str, name: str) -> dict[str, Any]:
    ...
    if not isinstance(document, dict):
        raise GrpoKServeRolloutError("Kubernetes resource response is invalid")
    return document


def _delete_grpo_resources(root: Path, *, check: bool = True) -> None:
    result = subprocess.run(
        [
            "kubectl",
            "--context",
    ...
    if check and result.returncode != 0:
        raise GrpoKServeRolloutError(f"GRPO resource cleanup failed: {result.stdout[-512:]}")
...
def _stop_worker(
    ...
    *,
    ...
    if _container_exists(root, WORKER_CONTAINER):
        ...
        if check and result.returncode != 0:
            raise GrpoKServeRolloutError(f"GRPO worker cleanup failed: {result.stdout[-512:]}")
    ...
    if check and _container_exists(root, WORKER_CONTAINER):
        raise GrpoKServeRolloutError("GRPO worker container still exists after cleanup")
...
def _stop_cluster(root: Path, *, check: bool = True) -> None:
    ...
    if check and result.returncode != 0:
        raise GrpoKServeRolloutError(f"Kind cluster stop failed: {result.stdout[-512:]}")
...
def _resolve_latest_outcome(root: Path, pointer_path: Path) -> Path:
    ...
    if not isinstance(pointer, dict):
        raise GrpoKServeRolloutError("GRPO runtime pointer is invalid")
    value = pointer.get("outcome") or pointer.get("report")
    if not isinstance(value, str) or not value:
        raise GrpoKServeRolloutError("GRPO runtime pointer has no outcome")
    path = (root / value).resolve(strict=True)
    try:
        path.relative_to(root)
    except ValueError as exc:
        raise GrpoKServeRolloutError("GRPO runtime pointer escaped repository") from exc
    return path
...
def _variant_count(document: dict[str, Any], field: str, variant: str) -> int:
    ...
    if not isinstance(values, dict):
        raise GrpoKServeRolloutError(f"runtime metric is invalid: {field}")
    value = values.get(variant)
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise GrpoKServeRolloutError(f"runtime metric is invalid: {field}.{variant}")
    return value
...
def _text(document: dict[str, Any], field: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise GrpoKServeRolloutError(f"field is invalid: {field}")
    return value
...
def _run_checked(argv: list[str], *, cwd: Path) -> None:
    ...
    if result.returncode != 0:
        raise GrpoKServeRolloutError(f"command failed ({argv[0]}): {result.stdout[-1_024:]}")
...
def _progress(stage: str) -> None:
    print(f"GRPO_KSERVE_ROLLOUT_STAGE={stage}", flush=True)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-grpo-kserve-rollout")
    parser.add_argument(
        "command",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run":
        ...
        report = verify_grpo_kserve_rollout(root, path)
        ...
        _stop_cluster(root)
        print("GRPO_KSERVE_RESOURCES_CLEANED")
        return 0
