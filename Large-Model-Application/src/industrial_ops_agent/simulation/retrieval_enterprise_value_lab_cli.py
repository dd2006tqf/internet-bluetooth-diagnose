"""CLI for the low-memory actual-GPU Retrieval enterprise-value lab."""
...
from industrial_ops_agent.simulation.retrieval_enterprise_value_lab import (
    ...
    IMAGE,
    OUTPUT_RELATIVE,
    RetrievalEnterpriseValueLabError,
    RetrievalEnterpriseValueReport,
    assert_gold_available,
    execute_worker,
    planned_run_id,
    verify_retrieval_enterprise_value,
)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-retrieval-enterprise-value-lab")
...
def run(argv: list[str] | None = None) -> None:
    ...
    if args.command == "worker":
        ...
        report = verify_retrieval_enterprise_value(root, path)
    elif args.command == "verify":
        report = verify_retrieval_enterprise_value(root)
    ...
    for evaluation in report.evaluations:
        print(
            f"{evaluation.component}_BASELINE_{evaluation.primary_metric.upper()}="
            f"{evaluation.baseline_primary:.6f}"
        )
        print(
            f"{evaluation.component}_CANDIDATE_{evaluation.primary_metric.upper()}="
            f"{evaluation.candidate_primary:.6f}"
        )
...
def _run_container(root: Path) -> tuple[RetrievalEnterpriseValueReport, Path]:
    ...
    if acceptance.is_file():
        return verify_retrieval_enterprise_value(root, acceptance), acceptance
    assert_gold_available(root)
    if _container_exists(root):
        raise RetrievalEnterpriseValueLabError(
            f"Retrieval lab container already exists: {CONTAINER_NAME}"
        )
    ...
    command = [
        ...
        "-B",
        "-m",
        "industrial_ops_agent.simulation.retrieval_enterprise_value_lab_cli",
        "worker",
        "--repo-root",
        "/workspace",
        "--image-digest",
    ...
    try:
        ...
        raise RetrievalEnterpriseValueLabError(
            "actual GPU Retrieval container failed; inspect retained rejection "
        ...
        if _container_exists(root):
            raise RetrievalEnterpriseValueLabError("Retrieval lab container cleanup failed")
    report = verify_retrieval_enterprise_value(root, acceptance)
...
def _remove_owned_container(root: Path) -> None:
    try:
        ...
        raise RetrievalEnterpriseValueLabError("Retrieval lab container cleanup failed") from exc
...
def _capture_checked(command: list[str], *, cwd: Path) -> str:
    try:
        ...
        raise RetrievalEnterpriseValueLabError(f"command failed: {command[0]}") from exc
    value = completed.stdout.strip()
    if not value:
        raise RetrievalEnterpriseValueLabError(f"command returned no output: {command[0]}")
    return value
