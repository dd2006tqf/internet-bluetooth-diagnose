"""CLI for the low-memory actual-GPU Embedding recovery value lab."""
...
from industrial_ops_agent.simulation.embedding_recovery_value_lab import (
    ...
    IMAGE,
    OUTPUT_RELATIVE,
    EmbeddingRecoveryValueLabError,
    EmbeddingRecoveryValueReport,
    assert_gold_available,
    execute_worker,
    planned_run_id,
    verify_embedding_recovery_value,
)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-embedding-recovery-value-lab")
...
def run(argv: list[str] | None = None) -> None:
    ...
    if args.command == "worker":
        ...
        report = verify_embedding_recovery_value(root, path)
    elif args.command == "verify":
        report = verify_embedding_recovery_value(root)
...
def _print_report(report: EmbeddingRecoveryValueReport, path: Path) -> None:
    print(report.status)
...
def _run_container(root: Path) -> tuple[EmbeddingRecoveryValueReport, Path]:
    ...
    if acceptance.is_file():
        return verify_embedding_recovery_value(root, acceptance), acceptance
    assert_gold_available(root)
    if _container_exists(root):
        raise EmbeddingRecoveryValueLabError(
            f"Embedding v2 lab container already exists: {CONTAINER_NAME}"
        )
    ...
    command = [
        ...
        "-B",
        "-m",
        "industrial_ops_agent.simulation.embedding_recovery_value_lab_cli",
        "worker",
        "--repo-root",
        "/workspace",
        "--image-digest",
    ...
    try:
        ...
        raise EmbeddingRecoveryValueLabError(
            "actual GPU Embedding v2 container failed; inspect retained rejection "
        ...
        if _container_exists(root):
            raise EmbeddingRecoveryValueLabError("Embedding v2 lab container cleanup failed")
    report = verify_embedding_recovery_value(root, acceptance)
...
def _remove_owned_container(root: Path) -> None:
    try:
        ...
        raise EmbeddingRecoveryValueLabError("Embedding v2 lab container cleanup failed") from exc
...
def _capture_checked(command: list[str], *, cwd: Path) -> str:
    try:
        ...
        raise EmbeddingRecoveryValueLabError(f"command failed: {command[0]}") from exc
    value = completed.stdout.strip()
    if not value:
        raise EmbeddingRecoveryValueLabError(f"command returned no output: {command[0]}")
    return value
