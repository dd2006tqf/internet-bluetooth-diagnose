"""Actual-GPU strong-ASR TTS enterprise-value lab v6.
...
The retired v8 suite is used only as a disclosed development regression set.
Its Whisper Tiny terminology miss is addressed by a fixed Whisper Small English
verifier while the latency-pruned 23-candidate policy remains unchanged.  The
fresh, disjoint, single-use v9 Gold is never opened during development.
"""
...
from industrial_ops_agent.simulation import tts_latency_value_lab as v5
...
PRIOR_GOLD_V8_RELATIVE = (
    LATENCY_OUTPUT_RELATIVE
    / "inputs/industrial-safety-tts-final-gold-v8/manifest.json"
)
PRIOR_GOLD_V8_RETIREMENT_RELATIVE = LATENCY_OUTPUT_RELATIVE / "gold-v8-retirement.json"
PRIOR_V8_FAILURE_RELATIVE = (
    LATENCY_OUTPUT_RELATIVE
    / "rejections/tts-latency-f1c6f42b8bc3f3fa5f34/gold-stage-failure.json"
)
PRIOR_V8_VALIDATION_RELATIVE = (
    LATENCY_OUTPUT_RELATIVE
    / "development/tts-latency-f1c6f42b8bc3f3fa5f34-validation-probe.json"
)
GOLD_RELATIVE = (
    OUTPUT_RELATIVE / "inputs/industrial-safety-tts-final-gold-v9/manifest.json"
)
GOLD_RETIREMENT_RELATIVE = OUTPUT_RELATIVE / "gold-v9-retirement.json"
...
ASR_RELATIVE = Path(
    "artifacts/m7-tts-asr-value-lab/inputs/whisper-small-en-e8727524"
)
...
CONTAINER_NAME = "ioap-tts-strong-asr-value-lab"
...
class TtsCalibrationValidationError(TtsCalibratedValueLabError):
    ...
    def __init__(self, document: dict[str, Any]) -> None:
        super().__init__("TTS strong-ASR stage failed its safe-candidate contract")
...
_VALIDATION_ROWS: tuple[dict[str, Any], ...] = (
    _RETIRED_GOLD_V8_ROWS + _STRONG_ASR_EXTRA_ROWS
)
...
def preflight_calibrated_tts(repo_root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "enterprise-tts-strong-asr-preflight/v8",
        "classification": CLASSIFICATION,
        "status": "TTS_STRONG_ASR_PREFLIGHT_PASSED",
...
def assert_gold_available(repo_root: Path) -> None:
    ...
    if retirement.exists():
        ...
        raise TtsCalibratedValueLabError("TTS Gold v9 is already retired")
...
def planned_run_id(repo_root: Path, image_digest: str) -> str:
    ...
    return f"tts-strong-asr-{_digest(identity)[:20]}"
...
def execute_worker(repo_root: Path, *, image_digest: str) -> Path:
    ...
    if accepted_path.parent.exists() or rejected_path.parent.exists():
        raise TtsCalibratedValueLabError("immutable TTS strong-ASR run directory exists")
    ...
    try:
        ...
        validation_document_value = {
            "schema_version": "enterprise-tts-strong-asr-validation/v8",
            "classification": CLASSIFICATION,
            "run_id": run_id,
        ...
        try:
            ...
            failure_document = {
                "schema_version": "enterprise-tts-strong-asr-gold-stage-failure/v1",
                "classification": CLASSIFICATION,
                "status": "TTS_STRONG_ASR_GOLD_NO_SAFE_VARIANT_REJECTED",
                "run_id": run_id,
        ...
        report_value: dict[str, Any] = {
            ...
            "status": (
                "TTS_STRONG_ASR_ACTUAL_GPU_ENTERPRISE_VALUE_PASSED"
                if accepted
                else "TTS_STRONG_ASR_ACTUAL_GPU_CANDIDATE_REJECTED"
            ),
            "decision": (
                "TTS_CANDIDATE_ELIGIBLE_FOR_MODEL_RELEASE_DRAFT"
                if accepted
            ...
            "dataset": {
                ...
                "project_enterprise_use_authorized": True,
                "enterprise_production_data": False,
                "retired_v2_v3_v4_v5_v6_v7_v8_reused_as_formal_gold": False,
                "retired_v8_used_as_disclosed_regression": True,
...
def execute_validation_probe(repo_root: Path, *, image_digest: str) -> Path:
    ...
    try:
        ...
        document = {
            "schema_version": "enterprise-tts-strong-asr-validation-probe/v8",
            "classification": CLASSIFICATION,
            "status": (
                "TTS_STRONG_ASR_GPU_VALIDATION_PASSED"
                if not result.failures
                else "TTS_STRONG_ASR_GPU_VALIDATION_REJECTED"
            ),
            "run_id": run_id,
        ...
        document = {
            "schema_version": "enterprise-tts-strong-asr-validation-probe/v8",
            "classification": CLASSIFICATION,
            "status": "TTS_STRONG_ASR_GPU_VALIDATION_REJECTED",
            "run_id": run_id,
            "actual_gpu_inference": True,
            "formal_gold_consumed": False,
            "failure": exc.document,
...
def _evaluate_stage(
    ...
    *,
    ...
    observations_document = {
        "schema_version": "enterprise-tts-strong-asr-observations/v4",
        "classification": CLASSIFICATION,
        "stage": stage,
        "selection_policy": "WHISPER_SMALL_SAFETY_THEN_XVECTOR_MAX",
        "candidate_pool_size": _candidate_pool_size(),
    ...
    formal_document_value = {
        "schema_version": "enterprise-tts-strong-asr-formal-evaluation/v4",
        "classification": CLASSIFICATION,
        "stage": stage,
...
def _transcribe_variants(
    ...
    torch = dependencies["torch"]
    transformers = dependencies["transformers"]
    asr_path = root / ASR_RELATIVE
    processor = transformers.AutoProcessor.from_pretrained(
        asr_path, local_files_only=True
    )
...
def _runtime_evidence(dependencies: dict[str, Any], peak: int) -> dict[str, Any]:
    ...
    return {
        ...
        "verifier_policy": "WHISPER_SMALL_SAFETY_THEN_XVECTOR_MAX",
...
def _materialize_candidate(
    ...
    policy = {
        ...
        "retired_gold_v8_used_as_disclosed_development_evidence": True,
        "formal_gold_v9_derived": False,
        "retired_gold_v8_regression_used": True,
        "asr_verifier": _asr_model_evidence(root).model_dump(mode="json"),
        "runtime_source": "industrial_ops_agent.simulation.tts_strong_asr_value_lab",
    }
...
def _prior_asr_evidence(root: Path) -> PriorAsrEvidence:
    failure_path = _inside_file(root, PRIOR_V8_FAILURE_RELATIVE)
    validation_path = _inside_file(root, PRIOR_V8_VALIDATION_RELATIVE)
    gold_path = _inside_file(root, PRIOR_GOLD_V8_RELATIVE)
    retirement_path = _inside_file(root, PRIOR_GOLD_V8_RETIREMENT_RELATIVE)
    ...
    if (
        ...
        != "enterprise-tts-latency-gold-stage-failure/v1"
        ...
        != "industrial-safety-tts-gold-retirement/v8"
        or retirement.get("status") != "RETIRED_AFTER_REJECTED_FINAL_EVALUATION"
        ...
        or retirement.get("reuse_permitted") is not False
        or retirement.get("gold_manifest_sha256") != _file_sha256(gold_path)
        or retirement.get("outcome_sha256") != _file_sha256(failure_path)
    ):
        raise TtsCalibratedValueLabError("TTS v8 ASR failure evidence changed")
    source_value = failure.get("source")
    source_location = v5.__file__
    if not isinstance(source_location, str):
        raise TtsCalibratedValueLabError("TTS v8 source location is unavailable")
    source_path = Path(source_location).resolve(strict=True)
    if source_value != _file_binding(root, source_path):
        raise TtsCalibratedValueLabError("TTS v8 source evidence changed")
    return PriorAsrEvidence(
        gold_stage_failure=FileBinding.model_validate(
            _file_binding(root, failure_path)
        ),
        validation_probe=FileBinding.model_validate(
            _file_binding(root, validation_path)
        ),
        gold_manifest=FileBinding.model_validate(_file_binding(root, gold_path)),
        gold_retirement=FileBinding.model_validate(
            _file_binding(root, retirement_path)
        ),
...
def _asr_model_evidence(root: Path) -> AsrModelEvidence:
    path = _inside_directory(root, ASR_RELATIVE)
    required = (
        "config.json",
    ...
    config = _load_object(path / "config.json")
    ...
    return AsrModelEvidence(
        bundle=DirectoryBinding.model_validate(_directory_binding(root, path)),
        model_id=ASR_MODEL_ID,
        revision=ASR_REVISION,
...
def _verify_dataset_independence(
    ...
    prior_documents = [
        ...
        _load_object(root / PRIOR_GOLD_V8_RELATIVE),
    ]
...
def _write_gold_retirement(
    ...
    document = {
        "schema_version": "industrial-safety-tts-gold-retirement/v9",
        "status": (
        ...
        "run_id": report.run_id,
        "gold_dataset_id": "industrial-safety-tts-final-gold-v9",
...
def _verify_gold_retirement(root: Path, path: Path) -> dict[str, Any]:
    ...
    if (
        retirement.get("schema_version") != "industrial-safety-tts-gold-retirement/v9"
        or retirement.get("gold_dataset_id") != "industrial-safety-tts-final-gold-v9"
        or retirement.get("gold_evaluation_pass_count") != 1
        or retirement.get("reuse_permitted") is not False
    ):
        raise TtsCalibratedValueLabError("TTS Gold v9 retirement changed")
    gold_path = _inside_file(root, GOLD_RELATIVE)
    if retirement.get("gold_manifest_sha256") != _file_sha256(gold_path):
        raise TtsCalibratedValueLabError("TTS Gold v9 retirement manifest changed")
    ...
    if isinstance(outcome_value, str):
        ...
        if retirement.get("outcome_sha256") != _file_sha256(outcome):
            raise TtsCalibratedValueLabError("TTS Gold v9 retirement outcome changed")
    return retirement
...
def _write_latest(root: Path, path: Path, report: TtsCalibratedValueReport) -> None:
    _write_json(
        ...
        {
            "schema_version": "enterprise-tts-strong-asr-latest/v1",
            "status": report.status,
            "run_id": report.run_id,
...
def _write_latest_outcome(
    ...
    _write_json(
        ...
        {
            "schema_version": "enterprise-tts-strong-asr-latest-outcome/v1",
            "status": report.status,
            "run_id": report.run_id,
