...
from industrial_ops_agent.simulation.tts_calibrated_value_lab import (
    CONTAINER_NAME,
    IMAGE,
    OUTPUT_RELATIVE,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-tts-calibrated-value-lab")
...
def _run_container(root: Path) -> tuple[TtsCalibratedValueReport, Path]:
    ...
    command = [
        ...
        "-B",
        "-m",
        "industrial_ops_agent.simulation.tts_calibrated_value_lab_cli",
        "worker",
        "--repo-root",
        "/workspace",
        "--image-digest",
    ...
    try:
        ...
        raise TtsCalibratedValueLabError("actual GPU TTS calibrated container failed") from exc
    finally:
        if _container_exists(root):
            _remove_owned_container(root)
        if _container_exists(root):
            raise TtsCalibratedValueLabError("TTS calibrated container cleanup failed")
...
def _remove_owned_container(root: Path) -> None:
    try:
        ...
        raise TtsCalibratedValueLabError("TTS calibrated container cleanup failed") from exc
