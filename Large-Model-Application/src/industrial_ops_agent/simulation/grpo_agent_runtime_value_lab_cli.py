"""CLI for the one-shot actual-GPU GRPO Agent Runtime value lab."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-grpo-agent-runtime-value-lab")
...
def run(argv: list[str] | None = None) -> None:
    ...
    if args.command == "prepare":
        ...
        print("GRPO_AGENT_RUNTIME_GOLD_FROZEN")
        print(path)
        return
    if args.command == "worker":
        path = execute_worker(root, image_digest=args.image_digest)
        report = verify_grpo_agent_runtime_value(root, path)
    elif args.command == "verify":
        report = verify_grpo_agent_runtime_value(root)
...
def _run_container(root: Path) -> tuple[GrpoAgentRuntimeValueReport, Path]:
    ...
    for path in (
        ...
        if path.is_file():
            return verify_grpo_agent_runtime_value(root, path), path
    if _container_exists(root):
        raise GrpoAgentRuntimeValueLabError(
            f"GRPO Agent Runtime lab container already exists: {CONTAINER_NAME}"
        )
    command = [
        "docker",
        "run",
        ...
        "-B",
        "-m",
        "industrial_ops_agent.simulation.grpo_agent_runtime_value_lab_cli",
        "worker",
        "--repo-root",
        "/workspace",
        "--image-digest",
    ...
    try:
        ...
        raise GrpoAgentRuntimeValueLabError(
            "actual GPU GRPO Agent Runtime container failed"
        ) from exc
    finally:
        if _container_exists(root):
        ...
        if _container_exists(root):
            raise GrpoAgentRuntimeValueLabError(
                "GRPO Agent Runtime lab container cleanup failed"
            )
    return verify_grpo_agent_runtime_value(root), root / LATEST_RELATIVE


def _print_report(report: GrpoAgentRuntimeValueReport, path: Path) -> None:
    print(report.status)
    print(report.classification)
    print(f"CANDIDATE_ACCEPTED={str(report.candidate_accepted).lower()}")
    print(
        "BASELINE_DIRECT_OBJECT_RATE="
        f"{report.evaluation.baseline.direct_object_rate:.6f}"
    )
    print(
        "CANDIDATE_DIRECT_OBJECT_RATE="
        f"{report.evaluation.candidate.direct_object_rate:.6f}"
    )
...
def _capture_checked(command: list[str], *, cwd: Path) -> str:
    try:
        ...
        raise GrpoAgentRuntimeValueLabError(f"command failed: {command[0]}") from exc
