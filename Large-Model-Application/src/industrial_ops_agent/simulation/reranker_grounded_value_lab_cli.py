"""CLI for the low-memory actual-GPU grounded Reranker value lab v4."""
...
from industrial_ops_agent.simulation.reranker_grounded_value_lab import (
    CONTAINER_NAME,
    IMAGE,
    OUTPUT_RELATIVE,
    RerankerGroundedValueLabError,
    RerankerGroundedValueReport,
    assert_gold_available,
    execute_worker,
    planned_run_id,
    preflight_grounded_reranker,
    prepare_inputs,
    verify_grounded_reranker_value,
)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="industrial-ops-reranker-grounded-value-lab")
...
def run(argv: list[str] | None = None) -> None:
    ...
    if args.command == "prepare":
        ...
        print("RERANKER_GROUNDED_INPUTS_PREPARED")
        print(registry_path)
        print(gold_path)
        return
    if args.command == "preflight":
        result = preflight_grounded_reranker(root)
        print(result["status"])
        print(f"SOURCE_V3_RUN_ID={result['source_v3_run_id']}")
        print(f"GOLD_CONSUMED={str(result['gold_consumed']).lower()}")
        return
    if args.command == "worker":
        path = execute_worker(root, image_digest=args.image_digest)
        report = verify_grounded_reranker_value(root, path)
    elif args.command == "verify":
        report = verify_grounded_reranker_value(root)
...
def _print_report(report: RerankerGroundedValueReport, path: Path) -> None:
    print(report.status)
...
def _run_container(root: Path) -> tuple[RerankerGroundedValueReport, Path]:
    ...
    for existing in (acceptance, rejection):
        if existing.is_file():
            return verify_grounded_reranker_value(root, existing), existing
    assert_gold_available(root)
    if _container_exists(root):
        raise RerankerGroundedValueLabError(
            f"Reranker v4 lab container already exists: {CONTAINER_NAME}"
        )
    ...
    command = [
        ...
        "-B",
        "-m",
        "industrial_ops_agent.simulation.reranker_grounded_value_lab_cli",
        "worker",
        "--repo-root",
        "/workspace",
        "--image-digest",
    ...
    try:
        ...
        raise RerankerGroundedValueLabError(
            "actual GPU Reranker v4 container failed; Gold is retired if evaluation began"
        ) from exc
    finally:
        if _container_exists(root):
            _remove_owned_container(root)
        if _container_exists(root):
            raise RerankerGroundedValueLabError("Reranker v4 lab cleanup failed")
    for outcome in (acceptance, rejection):
        if outcome.is_file():
            return verify_grounded_reranker_value(root, outcome), outcome
    raise RerankerGroundedValueLabError(
        "Reranker v4 worker produced no governed outcome"
    )
...
def _remove_owned_container(root: Path) -> None:
    try:
        ...
        raise RerankerGroundedValueLabError("Reranker v4 lab cleanup failed") from exc
...
def _capture_checked(command: list[str], *, cwd: Path) -> str:
    try:
        ...
        raise RerankerGroundedValueLabError(f"command failed: {command[0]}") from exc
    value = completed.stdout.strip()
    if not value:
        raise RerankerGroundedValueLabError(f"command returned no output: {command[0]}")
    return value
