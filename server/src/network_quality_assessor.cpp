/**
 * @file network_quality_assessor.cpp
 * @brief 网络质量综合评估器 — 将 RTT、TCP 丢包率、RSSI、流量特征四个维度加权融合为单一网络质量分数
 *
 * 模块职责：
 *   - 遍历所有 NetInfo 接口，找到"当前正在使用"的接口（UsingInterfaceManager）
 *   - 对活跃接口的 RTT / TCP 丢包率 / RSSI / 流量特征分别评分，再按权重合成总分
 *   - 根据总分映射质量等级（EXCELLENT / GOOD / FAIR / POOR）
 *   - 检测异常维度（高延迟 / 高丢包 / 弱信号 / 小包率）并生成 issues 列表
 *   - 提供 updateThresholds() 支持运行时调整阈值配置
 *
 * 评估维度与权重设计：
 *   ┌──────────────┬────────┬──────────────────────────────────────────────────────┐
 *   │ 维度          │ 权重   │ 评分逻辑                                               │
 *   ├──────────────┼────────┼──────────────────────────────────────────────────────┤
 *   │ RTT          │  30%   │ ≤50ms→100 / ≤100ms→80 / ≤200ms→60 / >200ms→20~60递减 │
 *   │ TCP 丢包率    │  30%   │ ≤0.1%→100 / ≤0.5%→80 / ≤2%→60 / >2%→10~60递减       │
 *   │ RSSI         │  20%   │ ≥-60dBm→100 / ≥-70→80 / ≥-80→60 / <-80→10~60递减     │
 *   │ 流量特征      │  20%   │ avgPacketSize>1KB→+20 / <200B→-20 / flows≥阈值→+10  │
 *   └──────────────┴────────┴──────────────────────────────────────────────────────┘
 *
 * 权重理由：RTT 和 TCP 丢包直接影响用户体验（网页加载卡顿/视频缓冲），各给 30%；
 * RSSI 反映链路质量但中间可能有多跳放大，给 20%；流量特征是辅助验证信号（异常流量模式），给 20%。
 *
 * 等级划分：
 *   EXCELLENT ≥90 / GOOD ≥75 / FAIR ≥50 / POOR <50 / UNKNOWN（无接口）
 *
 * 依赖的外部接口：
 *   - **network_quality_assessor.hpp** 定义 NetInfo 接口数据结构与 QualityThresholds 阈值配置
 *   - **net_info.hpp**                   NetInfo 的 RTT/RSSI/TCP loss/流量访问器
 */
